...
<% from MaKaC.fossils.user import IAvatarFossil %>
    ...
    var cleanTxt = function(txt) {
        var cleaned = '';
        try {
            cleaned = decodeURIComponent(escape(txt));
        }
        catch(err) {
            cleaned = txt;
        };
            ...
            <td>
                ...
                % elif option.getType() == "links":
                    ...
                    <script type="text/javascript">
                        ...
                        % elif option.getSubType() == 'sponsors':
                            addLinkText = $T('Add new Sponsor');
                            noLinksMsg = $T('No Sponsors created yet. Click "Add new Sponsor" to create one');
                            popupTitle = $T('Enter Sponsor\'s data');
                            linkNameLabel = $T('Code');
                            linkNameHeader = $T('Name');  
                           	var linkCountryLabel = $T('Country'); 
                           	var linkTitleLabel = $T('Title');                                                      
                           	var linkLogoLabel = $T('Logo');                             
                        % endif

                        var renderLinkTable = function(table) {
                            if(table.length > 0){
                                var linksBody = Html.tbody();
                                var linksTable = Html.table({style:{border:"1px dashed #CCC",width:"1000px",tableLayout: "fixed"}, id:"exportableTable"}, linksBody);
                                % if option.getSubType() == 'sponsors':
									linksBody.append( Html.tr({style: {marginTop: pixels(10)}}, Html.td({style:{width:pixels(100), whiteSpace: "nowrap", fontWeight:"bold", paddingRight:pixels(10)}}, linkNameHeader),
																								Html.td({style:{width:pixels(100), whiteSpace: "nowrap", fontWeight:"bold", paddingRight:pixels(10)}}, linkCountryLabel),
																								Html.td({style:{width:pixels(400), fontWeight:"bold", paddingRight:pixels(10)}}, linkTitleLabel),
																								Html.td({style:{width:pixels(200), whiteSpace: "nowrap", overflow:"auto", fontWeight:"bold"}}, $T('URL')),	
																								Html.td({style:{width:pixels(200), whiteSpace: "nowrap", fontWeight:"bold", paddingRight:pixels(10)}}, linkLogoLabel),
																								Html.td({style:{width:pixels(200), whiteSpace: "nowrap", fontWeight:"bold"}}, ''),
																								Html.td({style:{width:pixels(25), whiteSpace: "nowrap", fontWeight:"bold"}},''  ) ));		                            
                                ...
                                each(table, function(link){
                                        ...
                                        % if option.getSubType() == 'sponsors':
										    ...
										    if (link.logo) {
										        logoThumb = Html.img({src:"/css/ICTP/images/sponsor-logo/"+link.logo, style: {width:pixels(100),height:pixels(100)}});
										    }
	                                        var newRow = Html.tr({style: {marginTop: pixels(10),borderBottom:"1px solid #CCC"}}, Html.td({style: {width:pixels(100), marginRight: pixels(10), whiteSpace: "nowrap", paddingRight:pixels(20)}},link.name),
																								   Html.td({style: {width:pixels(100), marginRight: pixels(10), wordWrap:"break-word", paddingRight:pixels(20)}},cleanTxt(link.country)),
																								   Html.td({style: {width:pixels(400), marginRight: pixels(10), wordWrap:"break-word", paddingRight:pixels(20)}},cleanTxt(link.title)),
																								   Html.td({style: {width:pixels(200), marginRight: pixels(10), wordWrap:"break-word", paddingRight:pixels(20)}},link.structure),
																								   Html.td({style: {width:pixels(200), marginRight: pixels(10), wordWrap:"break-word", paddingRight:pixels(10)}},link.logo),
																								   Html.td({}, logoThumb),
																								   Html.td({style:{width:pixels(25), whiteSpace: "nowrap"}},removeButton));
										% else:
										...
										% endif                                                       Html.td({style:{whiteSpace: "nowrap"}},removeButton));
                        ...
                        addButton.observeClick(function() {
                            ...
                            % if option.getSubType() == 'sponsors':
                            	var linkCountry = new AutocheckTextBox({name: 'country', id:"linkcountry"});
	                            var linkTitle = new AutocheckTextBox({name: 'title', id:"linktitle"});
    	                        var self = this;
    	                        var upload = function() {
                                    var uploadAction = 'sponsor_management-upload';  
                                    var popup = new UploadLogoFileDialog('Upload Logo',{},'350px','30px',
                                    '',
                                    uploadAction,
                                    '',
                                    self.linkLogo);
                                ...
                                var linkLogo = Html.input('button',{}, $T('upload'));
        						self.linkLogo = linkLogo;
                				linkLogo.observeClick(upload);              				    	                        
	                            var div = $("<div></div>").append(IndicoUtil.createFormFromMap([
                                                                    [linkNameLabel, Html.div({}, linkName.draw(), errorLabel)],
                                                                    [linkCountryLabel, Html.div({}, linkCountry.draw())],                                                                    
                                                                    [linkTitleLabel, Html.div({}, linkTitle.draw())],
                                                                    [$T('URL'), Html.div({}, linkStructure)],
                                                                    [linkLogoLabel, Html.div({}, linkLogo)]]),
                                                              $("<div></div>").append(info),
                                    ...
                                    div,
                                                                        function(value){
                                                                            if(value){
																				...
																				% if option.getSubType() == 'sponsors':
																					p['country'] = linkCountry.get();
																					p['title'] = linkTitle.get();
																					p['logo'] = linkLogo.get();																					
																					if (linkLogo.get() == 'upload') { p['logo'] = ''; }
																					if (linkLogo.get().indexOf('File EXIST') > -1) { p['logo'] = ''; }
																				% endif 
							...
							% if option.getSubType() == 'sponsors':
								linksPopup.parameterManager.add(linkCountry, 'text', false);
								linksPopup.parameterManager.add(linkTitle, 'text', false);
								linksPopup.parameterManager.add(linkLogo, 'text', false);
							% endif 
                            linksPopup.open();
                        });
                        ...
                        var loadDefaultButton = Html.input("button", {style:{marginTop: pixels(5),marginLeft: pixels(5), float: "right"}}, 'Load Default');
                        
                        var confirmPopup = new ConfirmPopupWithPM('Confirm', '<div>Are you sure you want to load Defaults (all current values will be replaced) ? ',
                                function(value){
                                    if(value){
                                            var killProgress = IndicoUI.Dialogs.Util.progress($T("Loading Defaults..."));
                                            indicoRequest('sponsor_management.loadDefault',{ optionName: "${ name }" },
                                                function(result,error) {
                                                    if (!error && result.success) {
                                                        killProgress();
                                                        renderLinkTable(result.table);
                        ...
                        loadDefaultButton.observeClick(function() {
                            confirmPopup.open();                   
                        });


                        var exportLink = Html.a({href:'#', donwload:'export.csv', style:{marginLeft: pixels(5)}},$T("Export Table data into Excel"));
                        exportLink.observeClick(function() {
                            $("#exportableTable").toCSV();
                        });
...
<script>
    type("UploadLogoFileDialog", ["ExclusivePopupWithButtons"], {
        ...
        _fileUpload: function() {
            ...
            $(this.form.dom).ajaxForm({
                ...
                success: function(resp){
                    self.killProgress();
                    self.close();
                    self.query.dom.value = resp.info;
                    //self.onUpload(resp);
                }
            });
              ...
              tmpArr = [];
                 $(this).find("td").each(function() {
                      if($(this).text().match(/^-{0,1}\d*\.{0,1}\d+$/)) {
                          ...
                          tmpStr = decodeURIComponent($(this).text()).replace(/"/g, '""');
                          tmpArr.push('"' + tmpStr + '"');
                      }
                 });
