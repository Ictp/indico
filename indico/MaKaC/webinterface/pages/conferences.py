...
class WConfDisplayFrame(wcomponents.WTemplated):
    ...
    def getLogoPath(self, key):
        ...
        fname = key.replace(" ","").replace("-","").replace("_","").replace("/","").replace("'","").replace(".","").replace("(","").replace(")","")
        p = htdocsDir + "/css/ICTP/images/sponsor-logo/" + fname
        if os.path.isfile(p + "_logo.jpg"):
            return p + "_logo.jpg"
        if os.path.isfile(p + "_logo.png"):
            return p + "_logo.png"
    ...
    # Ictp :resize img
    def resizeImage(self, path, width, format='jpeg'):
        ftype = path.split(".")[-1].lower()
        #if ftype == 'gif': format = 'png'        
        if ftype in ['gif','png']: format = 'png'        
        img = Image(filename=path, resolution=72)        
        img.compression_quality = 85
        img.alpha_channel = True
        img.background_color = Color('white')
        img.transform(resize=width)        
        data = base64.b64encode(img.make_blob(format=format))
        return data
        ...
        if roles:
            try:
                ...
                if "Cosponsor(s)" in dict.keys():
                    dd = self.getSponsors(dict["Cosponsor(s)"])
                # sort
                keylist = dd.keys()
                keylist.sort()
                for k in keylist:
                    cosponsors.append(dd[k])                    
